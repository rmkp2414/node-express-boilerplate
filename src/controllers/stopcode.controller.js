...
const { userService,fileService,stopcodeService } = require('../services');
...
const getStopCodes = catchAsync(async (req, res) => {
  // const filter = pick(req.query, ['name', 'role']);
  // const options = pick(req.query, ['sortBy', 'limit', 'page']);
  // const result = await stopcodeService.queryUsers(filter, options);
  console.log(req)
  const result = await stopcodeService.queryStopCodes();
  res.send(result);
});
